...
void LiteExecutor::Forward(bool is_train) {
  RunOps(is_train, 0, num_forward_nodes_);
}

void LiteExecutor::PartialForward(bool is_train, int step, int *step_left) {
...
void LiteExecutor::Backward(const std::vector<NDArray>& head_grads) {
...
void LiteExecutor::Print(std::ostream &os) const {  // NOLINT(*)
...
void LiteExecutor::SetMonitorCallback(const MonitorCallback& callback) {
...
nnvm::Graph LiteExecutor::InitFullGraph(
    nnvm::Symbol symbol,
...
void LiteExecutor::Init(nnvm::Symbol symbol,
                         const Context& default_ctx,
                         const std::map<std::string, Context>& ctx_map,
                         const std::vector<NDArray>& in_args,
                         const std::vector<NDArray>& arg_grad_store,
...
void LiteExecutor::InitDataEntryMemory(std::vector<NDArray>* shared_pool) {
...
void LiteExecutor::ExecuteMonCallback(size_t nid) {
...
void LiteExecutor::RunOps(bool is_train, size_t topo_start, size_t topo_end) {
  // Update context
  const auto& idx = graph_.indexed_graph();
...
LiteExecutor::CachedSegOpr LiteExecutor::CreateCachedSegOpr(size_t topo_start, size_t topo_end) {
  std::vector<Engine::VarHandle> use_vars;
  std::vector<Engine::VarHandle> mutate_vars;
  Context *pctx = nullptr;
  LiteExecutor::CachedSegOpr ret;
